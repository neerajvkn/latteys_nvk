...
def get_data(conditions, filters):
	data = frappe.db.sql("""
		SELECT
			...
			soi.item_name as item_name, soi.item_group as item_group,
			...
			( 
				...
				AND cm.reference_name = so.name
				AND cm.reference_doctype = 'Sales Order' ORDER BY cm.modified DESC LIMIT 1  ) as comment
		FROM
			`tabSales Order` so,
